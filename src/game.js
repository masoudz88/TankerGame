...
let gameData = {
  TANK_BARREL_GAP_Y: 120,
  TANK_BARREL_GAP_X: 90,
  TANK_MOVE_INTERVALS: 10,
  TANK_AIM_MOVE_INTERVALS: 10,
  UP_MOVEMENT_ON_EACH_CLICK: 5,
  IMAGE_WIDTH: 150,
  IMAGE_HEIGHT: 150,
  TANK_BARREL_ENDPOINT_GAP_X: 65,
  TANK_BARREL_ENDPOINT_GAP_y: 100,
  TANK_BALL_GAP_X_1: 90,
  TANK_BALL_GAP_X_2: 115,
  TANK_BALL_GAP_Y: 20,
  TANK_BALL_CLEAR_X_1: 70,
  TANK_BALL_CLEAR_X_2: 200,
  TANK_BALL_CLEAR_Y: 60,
  X_AXIS_SPEED: 2,
  Y_AXIS_SPEED: 2,
  GRAVITY: 0.08,
};
...
class Tank {
  constructor(x, y, direction) {
    ...
    this.aim_end_point_x = this.aim_start_point_x + 40;
    this.aim_start_point_y = this.y + gameData.TANK_BARREL_GAP_Y; // TODO: magic variable -> constants
    this.aim_end_point_y = this.aim_start_point_y - 100;
  ...
  drawImage() {
    ctx.drawImage(
      ...
      this.x,
      this.y,
      gameData.IMAGE_WIDTH,
      gameData.IMAGE_HEIGHT
    );
  }
  ...
  drawTank() {
    ...
    if (this.direction == "left") {
      ctx.lineTo(this.aim_end_point_x, this.aim_end_point_y);
    } else if (this.direction == "right") {
      ctx.lineTo(this.aim_end_point_x - 120, this.aim_end_point_y);
    }
    ctx.lineWidth = 10;
    ctx.stroke();
  }
  ...
  shooting() {
    ...
    if (this.direction == "left") {
      ...
      if (
        this.ball_start_point > c.width ||
        this.ball_end_point > this.y + 100
      ) {
      ...
      this.ball_start_point -= gameData.X_AXIS_SPEED * 8;
      if (this.ball_start_point < 100 || this.ball_end_point > this.y + 100) {
        // if ball goes beyond canvas
        return;
      }
    }

    if (this.ball_end_point < c.height / 2) {
      // if ball hits the height of canvas
...
function collision() {
  if (
    turn == 0 &&
    tank1.ball_start_point > tank2.x - 200 &&
    tank1.ball_start_point < tank2.x &&
    tank1.ball_end_point > tank2.y &&
    tank1.ball_end_point < tank2.y + 15
  ) {
    return true;
  } else if (
    turn == 1 &&
    tank2.ball_start_point > tank1.x + 230 &&
    tank2.ball_start_point < tank1.x + 350 &&
    tank2.ball_end_point > tank1.y &&
    tank2.ball_end_point < tank1.y + 15
  ) {
    return true;
  }
}
